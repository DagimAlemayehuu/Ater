...
impl ModelEngine {
    ...
    pub fn init(model_dir: &Path) -> Result<Self, String> {
        ...
        if !tokenizer_path.exists() {
            return Err(format!("tokenizer.json does not exist at {:?}", tokenizer_path));
        }
        ...
        if let Some(params) = tokenizer.get_truncation() {
            ...
            tokenizer.with_truncation(Some(new_params))
    ...
    pub fn get_embedding(&mut self, text: &str) -> Result<Vec<f32>, String> {
        ...
        let view = output_tensor.into_dimensionality::<ndarray::Ix3>()
...
mod tests {
    ...
    fn test_ml_inference() {
        ...
        let mut engine = ModelEngine::init(&model_dir)
            .expect("Failed to initialize ModelEngine for testing");

        let text = "test inference";
        let embedding = engine.get_embedding(text)
            .expect("Failed to generate embedding");

        println!("Successfully generated embedding of length {}", embedding.len());
    ...
    fn test_ml_edge_cases() {
        ...
        let empty_embedding = engine.get_embedding("")
        ...
        let massive_embedding = engine.get_embedding(&massive_text)
