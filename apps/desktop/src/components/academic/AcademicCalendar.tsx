...
const EVENT_COLORS: Record<string, { bg: string; border: string; text: string; dot: string; glow: string }> = {
  'Exam': { bg: 'bg-zinc-800', border: 'border-zinc-700', text: 'text-zinc-100', dot: 'bg-zinc-100', glow: 'shadow-none' },
  'Assignment': { bg: 'bg-zinc-900', border: 'border-zinc-800', text: 'text-zinc-300', dot: 'bg-zinc-400', glow: 'shadow-none' },
  'Study Session': { bg: 'bg-zinc-900/50', border: 'border-zinc-800', text: 'text-zinc-400', dot: 'bg-zinc-500', glow: 'shadow-none' },
  'Study': { bg: 'bg-zinc-900/50', border: 'border-zinc-800', text: 'text-zinc-400', dot: 'bg-zinc-500', glow: 'shadow-none' },
  'Practice': { bg: 'bg-zinc-900/50', border: 'border-zinc-800', text: 'text-zinc-400', dot: 'bg-zinc-500', glow: 'shadow-none' },
  'Note Visit': { bg: 'bg-zinc-900/30', border: 'border-zinc-800/50', text: 'text-zinc-500', dot: 'bg-zinc-600', glow: 'shadow-none' }
};
  ...
  return (
    <div className="flex flex-col gap-2 p-4">
      ...
      <div className="grid grid-cols-7 text-center mb-1">
        {weekDays.map((d, i) => <div key={i} className="text-[9px] font-bold text-zinc-500/30">{d}</div>)}
      </div>
      ...
      <div className="flex border-b border-border/20 pl-14 shrink-0">
        {days.map((day, i) => (
          <div key={i} className="flex-1 flex flex-col items-center justify-center py-4 border-l border-border/10 first:border-l-0 min-w-0">
            <span className={cn("text-[9px] font-black uppercase tracking-[0.2em]", isToday(day) ? "text-foreground" : "text-zinc-500/40")}>
              {format(day, 'EEE')}
            </span>
            <button 
        ...
        <div className="flex min-h-[1536px]">
          <div className="w-14 flex flex-col border-r border-border/10 sticky left-0 bg-background z-20">
            {hours.map(hour => (
              <div key={hour} className="h-16 flex justify-end pr-2.5 pt-2">
                <span className="text-[9px] font-black uppercase text-zinc-500/20 tracking-tighter">
          ...
          <div className="flex-1 flex relative">
            <div className="absolute inset-0 flex flex-col pointer-events-none">
              {hours.map(h => (
                <div key={h} className="h-16 border-t border-zinc-900/50 w-full" />
              ))}
            </div>
            ...
            {days.map((day, dayIdx) => {
              ...
              return (
                <div key={dayIdx} className="flex-1 relative border-l border-zinc-900/50 first:border-l-0 min-w-0">
                  {dayEvents.map((ev, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {height >= 45 && (
                          <div className={cn("text-[8px] truncate font-black uppercase tracking-widest text-zinc-500 pl-2.5")}>
                            {format(date, 'h:mm a')}
                          </div>
                        )}
                      </motion.div>
  ...
  return (
    <div className="flex-1 flex flex-col">
      <div className="grid grid-cols-7 border-b border-zinc-900">
        {weekDays.map(d => (
          <div key={d} className="py-2.5 text-center text-[10px] font-black tracking-[0.2em] text-zinc-500/30">
            {d}
          </div>
        ))}
      </div>
      <div className="flex-1 grid grid-cols-7 auto-rows-fr bg-zinc-950/50">
        {days.map((day, i) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar-mini flex flex-col gap-1">
                ...
                {dayEvents.length > 4 && (
                  <div className="text-[8px] font-black text-zinc-500/40 pl-1 uppercase tracking-widest">
                    + {dayEvents.length - 4}
                  </div>
                )}
              </div>
          ...
          return (
            <div key={month.getMonth()} className="flex flex-col">
              ...
              <div className="grid grid-cols-7 text-center mb-3">
                {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => <span key={i} className="text-[9px] font-black text-zinc-500/20">{d}</span>)}
              </div>
      ...
      <div className="w-64 border-r border-border/20 flex flex-col bg-zinc-950">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar py-2">
          ...
          <div className="px-6 py-4 flex flex-col gap-2 border-t border-border/10 mt-2">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500/30">Filters</span>
              <Filter size={10} className="text-zinc-500/20" />
            </div>
            {Object.entries(activeFilters).map(([name, active]) => {
              const color = "bg-zinc-500";
              return (
                <button 
                  key={name}
