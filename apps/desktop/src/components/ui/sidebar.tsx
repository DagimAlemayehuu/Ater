  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          'relative w-[var(--sidebar-width)] bg-transparent transition-[width]  ease-linear',
      ...
      <div
        ...
        className={cn(
          'fixed inset-y-0 z-10 hidden h-svh w-[var(--sidebar-width)] transition-[inset-inline,width]  ease-linear md:flex',
          side === 'left'
...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  return (
    <button
      ...
      className={cn(
        'absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2  ease-linear group-data-[side=left]:-end-4 group-data-[side=right]:start-0 after:absolute after:inset-y-0 after:start-1/2 after:w-[2px] hover:after:bg-sidebar-border sm:flex',
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        'flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70 ring-sidebar-ring outline-hidden transition-[margin,opacity]  ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
...
function SidebarMenuSkeleton({
  ...
  // We use a simple hash-like stable random for this instance if needed, 
  // but for a skeleton, a fixed 80% is often cleaner or we can just ignore the lint if it was harmless.
  // Let's use a stable value to satisfy the linter.
  const width = "80%"
