...
export function PracticeConfigurator({
  ...
  return (
    <div className="h-full flex flex-col bg-transparent font-sans overflow-hidden">
      <div className="flex-1 overflow-hidden flex flex-col p-10 pb-24">
        <div className="flex items-center justify-end mb-8 border-b border-border/40 pb-5">
          <div className="text-xl font-sans font-black tracking-tight text-foreground">{totalQuestions} Questions</div>
        </div>

        <div data-tour="practice-config-panel" className="grid grid-cols-1 lg:grid-cols-4 gap-6 flex-1 min-h-0">
          <div className="col-span-1 space-y-4 overflow-y-auto pr-2 custom-scrollbar">
            <div className="p-5 bg-bento-card border border-border/40 rounded-[12px] space-y-5 shadow-sm">
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent className="border-border/40 bg-bento-panel">
                    <SelectItem value="all" className="text-[10px] font-black uppercase tracking-tight text-foreground/80 hover:bg-bento-item">
                  ...
                  {[{ val: 'L1', label: '1' }, { val: 'L2', label: '2' }, { val: 'L3', label: '3' }, { val: 'Mixed', label: 'M' }].map(
                    (level) => (
                      <div key={level.val}>
                        ...
                        <Label
                          htmlFor={level.val}
                          className="flex h-10 border border-border/40 rounded-[8px] bg-bento-card peer-data-[state=checked]:bg-bento-item peer-data-[state=checked]:border-foreground/35 peer-data-[state=checked]:text-foreground items-center justify-center cursor-pointer text-[10px] font-black hover:bg-bento-item/20 hover:border-foreground/20 transition-all font-sans"
                        >
                          {level.label}
                        </Label>
                      </div>
              ...
              <div className="space-y-2">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <button
                      className="w-full h-10 border border-border/40 bg-bento-card hover:bg-bento-item/20 hover:border-foreground/20 text-[10px] font-black uppercase px-4 justify-between rounded-[6px] transition-all flex items-center cursor-pointer font-sans text-foreground"
                    >
                  ...
                  <PopoverContent
                    className="w-[300px] p-0 rounded-[12px] border border-border/40 bg-bento-panel overflow-hidden"
                    align="start"
                  >
                    <Command className="bg-transparent">
                      <div className="p-3.5 border-b border-border/40 flex justify-between items-center bg-bento-item/50">
                            ...
                            >
                              <div
                                className={cn(
                                  ...
                                  isSelected ? 'bg-bento-item border-foreground/35 text-foreground' : 'border-border/40'
                                )}
                              >
                                {isSelected && <Check size={8} className="text-foreground" strokeWidth={4} />}
                              </div>
          ...
          <div className="col-span-1 lg:col-span-3 p-8 bg-bento-card border border-border/40 rounded-[12px] flex flex-col min-h-0 shadow-sm">
            <div className="flex items-center justify-between border-b border-border/40 pb-4 mb-6 gap-4">
                ...
                {Object.entries(PRESETS).map(([k, p]) => (
                  <button
                    ...
                    className={cn(
                      ...
                      advancedConfig.smartMixPreset === k && "bg-bento-item text-foreground border-foreground/35 hover:bg-bento-item/80"
                    )}
                    title={p.label}
                  >
                    {p.label}
              ...
              <button
                ...
                className="h-7 px-2.5 text-[8px] font-black uppercase border border-border/40 bg-transparent hover:bg-bento-item rounded-[4px] flex items-center justify-center cursor-pointer text-foreground font-sans shrink-0"
              >
                Random
              </button>
            </div>
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 content-start">
              {[
                ...
                { key: 'writing', label: 'Explain' },
                { key: 'fill_in', label: 'Blank' },
                { key: 'debug', label: 'Debug' },
                { key: 'trace', label: 'Trace' },
                { key: 'order', label: 'Order' },
                { key: 'matching', label: 'Match' },
                { key: 'synthesis', label: 'Synthesize' },
                { key: 'calculation', label: 'Solve' },
                { key: 'data_analysis', label: 'Interpret' },
                { key: 'scenario', label: 'Scenario' },
                { key: 'code', label: 'Code' },
              ].map((type) => (
                <div
                  key={type.key}
                  className="space-y-2.5 p-4 bg-bento-card border border-border/40 hover:border-foreground/25 hover:bg-bento-item/5 transition-all rounded-[12px] shadow-sm"
                >
                  <div className="flex justify-between items-center">
                    <Label className="text-[8.5px] font-black uppercase tracking-widest text-muted-foreground/60">
                      {type.label}
                    </Label>
            ...
            <div className="flex gap-4 mt-6">
              <button
                ...
                className="h-11 flex-1 border border-border/40 hover:bg-bento-item text-[10px] font-black uppercase tracking-widest rounded-[6px] transition-all flex items-center justify-center cursor-pointer text-muted-foreground hover:text-foreground font-sans shadow-sm"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isLoading}
                className="h-11 flex-1 bg-bento-item border border-foreground/20 text-foreground hover:bg-bento-item/80 text-[10px] font-black uppercase tracking-widest rounded-[6px] transition-all flex items-center justify-center cursor-pointer disabled:opacity-30 disabled:pointer-events-none font-sans shadow-sm"
              >
                Start Practice
              </button>
            </div>
