           ...
           <div className="flex items-center gap-1 bg-muted/20 p-1 rounded-xl border border-border">
             {(['OVERALL', 'NOTES', 'PRACTICE'] as const).map(tab => (
               <button 
                 ...
                 className={cn(
                   "px-4 h-7 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all",
                   activeTab === tab 
          ...
          <div className="flex-1 flex p-10 gap-10">
            <div className="flex-1 flex flex-col space-y-10 overflow-hidden">
              ...
              <div className="grid grid-cols-2 gap-5">
                <div className="bg-muted/10 border border-border/60 rounded-2xl p-6 flex flex-col gap-2">
                ...
                <div className="h-full w-full pt-6">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={stats.barData}>
                      ...
                      <Tooltip cursor={{ fill: 'currentColor', fillOpacity: 0.02 }} contentStyle={{ backgroundColor: 'var(--background)', border: '1px solid var(--border)', borderRadius: '12px', fontSize: '10px' }} />
                      <Bar dataKey="value" fill="currentColor" fillOpacity={0.6} radius={[2, 2, 0, 0]} barSize={24} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
            ...
            <div className="w-[320px] flex flex-col space-y-6">
              ...
              <div className="flex-1 space-y-4 overflow-y-auto custom-scrollbar pr-2">
                {stats.categories.map((cat) => (
                  <div key={cat.name} className="bg-muted/10 border border-border/60 rounded-2xl p-6">
                    ...
                    <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden">
                      <motion.div className="h-full bg-foreground/20" initial={{ width: 0 }} animate={{ width: `${cat.percent}%` }} />
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab === 'NOTES' && (
          <div className="flex-1 flex flex-col p-10 gap-8 overflow-hidden">
            ...
            <div className="flex-1 grid grid-cols-2 gap-10 overflow-hidden">
               <div className="bg-muted/5 border border-border/40 rounded-2xl p-10 flex flex-col">
                 ...
                 <div className="flex-1">
                   <ResponsiveContainer width="100%" height="100%">
                     <BarChart data={stats.noteData} layout="vertical" margin={{ left: 40 }}>
                       ...
                       <Bar dataKey="value" fill="currentColor" fillOpacity={0.6} radius={[0, 2, 2, 0]} />
                     </BarChart>
                   </ResponsiveContainer>
                 </div>
               </div>
                 ...
                 <div className="flex-1">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={stats.practiceData}>
                        ...
                        <Bar dataKey="score" fill="currentColor" fillOpacity={0.6} radius={[2, 2, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                 </div>
               </div>
