...
import { ArrowLeft, RefreshCw, Plus, Search, Table, Kanban, LayoutGrid, Filter, ArrowUpDown, MoreVertical, FileText, ChevronDown, Calendar, Settings } from 'lucide-react'
...
export default function ObsidianDatabaseView({ database, onBack, onNavigate, onRefresh, initialSelectedRowId }: ObsidianDatabaseViewProps) {
    ...
    const [activeTab, setActiveTab] = useState<'table' | 'board' | 'gallery' | 'calendar'>('table')
    ...
    const saveCurrentView = (updates: any) => {
        setSavedViews(prev => {
            ...
            localStorage.setItem(`lifeos_db_views_${database.id}`, JSON.stringify({ views: newViews, activeId: activeViewId }));
            return newViews;
        });
    };
    ...
    const handleSelectView = (viewId: string) => {
        ...
        setFilters(view.filters || []);
        setGroupBy(view.groupBy || null);
        localStorage.setItem(`lifeos_db_views_${database.id}`, JSON.stringify({ views: savedViews, activeId: viewId }));
    };
    ...
    return (
        <div className="flex flex-col h-full w-full select-none bg-white font-sans text-[#111827]">
            ...
            <div className="flex flex-col px-4 pt-4 pb-0 shrink-0 border-b border-[#E5E5E5]">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3 text-[12px] font-medium text-gray-500">
                        ...
                        <span className="text-gray-900 font-semibold">{database.name}</span>
                    </div>
                ...
                <div className="flex items-center gap-1 text-[13px] font-medium text-gray-500 overflow-x-auto custom-scrollbar pb-[-1px] relative">
                    {savedViews.map(view => {
                        const Icon = view.activeTab === 'table' ? Table : view.activeTab === 'board' ? Kanban : view.activeTab === 'calendar' ? Calendar : LayoutGrid;
                        return (
                            <div key={view.id} className="relative flex items-center group/tab">
                                <button 
                                    ...
                                    className={cn(
                                        "flex items-center gap-2 px-4 py-2 border-b-2 transition-all cursor-pointer hover:text-black",
                    ...
                    <button onClick={handleAddView} className="px-3 py-2 text-gray-400 hover:text-black transition-colors" title="New View"><Plus size={14} /></button>
                </div>
            </div>
            ...
            <DatabaseSettingsPanel 
                ...
                filters={filters}
                onFiltersChange={(f) => { setFilters(f); saveCurrentView({ filters: f }); }}
                groupBy={groupBy}
