                ...
                {selectedCourse && (
                    <div className="px-6 py-2.5 border-b border-border/5 flex items-center gap-5 shrink-0 bg-muted/[0.03]">
                        ...
                        <div className="flex items-center gap-4 text-[8px] font-black uppercase tracking-widest text-muted-foreground/40">
                            ...
                            <span className="text-emerald-400">{visibleHubs.filter(h => stripWL(getVal(h, 'status', 'Status')).toLowerCase().includes('complet')).length} done</span>
                            <span className="text-blue-400">{visibleHubs.filter(h => stripWL(getVal(h, 'status', 'Status')).toLowerCase().includes('progress')).length} in progress</span>
                        </div>
                    </div>
                )}
...
function HubCard({ hub, onOpen, onPractice, onSetStatus, onSetStudyDate }: {
    ...
    return (
        <div className={cn(
            ...
            isCompleted ? 'border-emerald-500/10 bg-emerald-500/[0.02]' : 'border-border/20 bg-background hover:border-foreground/10'
        )}>
            {/* Header */}
