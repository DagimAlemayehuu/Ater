...
export default function CoursesTab({ data, databases, onUpdate, onCreate, onDelete, onOpenNote, navigateTo, initialSelectedId, onClearSelection }: TabProps) {
        ...
        return (
            <div className="h-full overflow-y-auto custom-scrollbar p-6 lg:p-10 space-y-10 pb-24">
                ...
                <div className="flex items-start justify-between">
                    ...
                    <button onClick={() => onDelete('07 - Courses', selectedId)} className="p-2 text-muted-foreground/20 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-all">
                        <Trash2 size={14} />
                    </button>
                </div>
                ...
                {/* Next exam callout */}
                {nextExam && (
                    <div className="p-4 bg-foreground/5 border border-border/20 rounded-xl flex items-center justify-between">
                        <div>
                        ...
                        <div className="text-right">
                            <span className="text-2xl font-black text-primary">{nextExam.date ? differenceInDays(new Date(nextExam.date), now) : '--'}</span>
                                ...
                                return (
                                    <div key={idx} onClick={() => onOpenNote(`3-Database/06 - Study Planner/${hub.id}.md`)}
                                        className={cn('p-3 border rounded-xl flex items-center gap-3 cursor-pointer hover:border-foreground/10 transition-all',
                                            isDone ? 'border-emerald-500/15 bg-emerald-500/[0.02]' : 'border-border/10 bg-background')}>
                                        <div className={cn('w-3 h-3 rounded-full border shrink-0', isDone ? 'bg-emerald-500 border-emerald-500' : 'border-border/30')} />
    ...
    return (
        <div className="h-full flex flex-col overflow-hidden">
            {/* Filter bar */}
            <div className="px-6 py-3 border-b border-border/10 flex items-center gap-3 shrink-0">
                ...
                <div className="flex items-center gap-2 flex-1 bg-muted/5 border border-border/10 rounded-lg px-3 py-2">
                    ...
                    {filtered.map((course, idx) => {
                        ...
                        return (
                            <div key={idx} onClick={() => setSelectedId(course.id)}
                                ...
                                {/* Quick stats */}
                                <div className="flex items-center gap-3 text-[8px] font-black uppercase tracking-widest border-t border-border/5 pt-2">
                                    {pending > 0 && <span className="text-amber-400">{pending} due</span>}
                                    {daysToExam !== null && daysToExam >= 0 && (
                                        <span className={daysToExam <= 7 ? 'text-red-400' : 'text-muted-foreground/30'}>exam in {daysToExam}d</span>
                                    )}
