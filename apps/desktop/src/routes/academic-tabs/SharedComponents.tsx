...
export function AcademicRoadmap({items, semesters = [], activeId, onSelect}: {
 ...
 className={cn(
    "group flex items-center justify-between p-3 rounded-lg border  cursor-pointer",
        ...
        {yearSemesters.map(s => {
            ...
            return (
                <div key={s.id} className="flex items-center justify-between group/sem">
                    ...
                    {sActive && <div className="w-1 h-1 rounded-full bg-foreground/70" />}
                </div>
            )
        })}
    </div>
...
export function SelectPropertyEditor({value, source, onSave, onCancel, label: displayLabel}: {
 ...
 'px-3 py-1.5 rounded-md text-[10px] font-black uppercase text-left ',
...
}} className="w-full mt-2 py-2 border border-dashed border-border rounded-lg text-[8px] font-black uppercase text-muted-foreground  hover:bg-muted/10  flex items-center justify-center gap-2">
...
export function ProgramSetupForm({onScaffold}: {
 ...
 className="w-full py-3.5 text-foreground border border-border bg-background text-[10px] font-black uppercase tracking-widest rounded-xl hover:border-foreground/50 disabled:opacity-20 ">
 Deploy Roadmap
 </button>
 </div>
 </div>
