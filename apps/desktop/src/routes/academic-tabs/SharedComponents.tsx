...
export function EmptyState({message, icon}: {message: string; icon?: React.ReactNode}) {
 return (
 <div className="py-16 text-center border border-border rounded-2xl w-full bg-muted/5 flex flex-col items-center gap-3">
...
export function StatCard({label, value, accent, onClick}: {label: string; value: string | number; accent?: boolean; onClick?: () => void}) {
 return (
 <div onClick={onClick} className={cn('p-6 border border-border rounded-2xl bg-muted/5 flex flex-col gap-2 transition-all', onClick && 'cursor-pointer hover:bg-muted/10 hover:border-foreground/10')}>
...
export function BigPropertyCard({label, value, schema, onUpdate}: {
 ...
 return (
 <div className="p-6 border border-border rounded-2xl bg-muted/5 flex flex-col justify-between gap-6 hover:border-foreground/10 transition-all">
 ...
 <button onClick={() => handleSave(!isChecked)} className={cn('w-8 h-8 flex items-center justify-center rounded-xl transition-all', isChecked ? 'text-foreground border border-foreground bg-muted/5' : 'bg-muted/5 border border-border hover:bg-muted/10')}>
 ...
 return (
 <div className="p-6 border border-border rounded-2xl bg-muted/5 flex flex-col gap-3 group/bigprop relative hover:bg-muted/10 hover:border-foreground/20 transition-all cursor-pointer"
...
export function CreateBanner({label, onConfirm, onCancel}: {
 ...
 return (
 <div className="mx-6 lg:mx-10 mb-4 p-3 bg-muted/5 border border-border rounded-xl flex items-center gap-3 ">
...
export function SelectPropertyEditor({value, source, onSave, onCancel, label: displayLabel}: {
 ...
 return (
 <div className="absolute top-full left-0 mt-1 w-full min-w-[220px] bg-background border border-border rounded-xl z-50 p-2 -95"
...
}} className="w-full mt-2 py-2 border border-dashed border-border rounded-lg text-[8px] font-black uppercase text-muted-foreground/40 hover:text-foreground hover:bg-muted/10 transition-all flex items-center justify-center gap-2">
...
export function ProgramSetupForm({onScaffold}: {
 ...
 return (
 <div className="p-10 border border-border rounded-2xl bg-muted/5 flex flex-col items-center justify-center text-center space-y-8 -95">
 ...
 className="w-full bg-background border border-border px-4 py-3 rounded-xl text-sm font-bold outline-none focus:ring-1 focus:ring-primary/20" />
 </div>
 <div className="grid grid-cols-3 gap-3">
 <div className="flex flex-col gap-1">
 <label className="text-[8px] font-black uppercase tracking-widest text-muted-foreground/40">Level</label>
 <select value={level} onChange={e => setLevel(e.target.value)}
 className="bg-background border border-border px-3 py-3 rounded-xl text-[10px] font-black uppercase outline-none">
