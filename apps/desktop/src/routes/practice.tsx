      ...
      <div className="h-full flex-1 flex flex-row w-full bg-background text-foreground animate-in fade-in duration-500 font-sans overflow-hidden">
        <div className="w-64 border-r border-border bg-muted/20 flex flex-col shrink-0">
          <div className="p-6 space-y-8">
            ...
            <div className="space-y-1">
              <button onClick={() => setView('dashboard')} className="w-full flex items-center gap-3 px-4 py-2 bg-background border border-border rounded-md text-[10px] font-black uppercase tracking-widest text-foreground transition-all"><LayoutGrid size={12}/> Dashboard</button>
          ...
          <div className="p-4 border-t border-border">
            <Button onClick={() => setView('configuring')} className="w-full h-11 bg-primary text-primary-foreground rounded-md font-black uppercase tracking-widest text-[10px] shadow-sm active:scale-[0.98] transition-all">Start Practice</Button>
          </div>
        </div>
        <div className="flex-1 overflow-y-auto custom-scrollbar p-10 space-y-16">
          <div className="flex items-end justify-between border-b border-border pb-10">
            <div className="space-y-2"><span className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-widest">Overview</span><h1 className="text-4xl font-black tracking-tighter uppercase leading-none text-foreground">Practice<br/><span className="text-muted-foreground/30">Analytics</span></h1></div>
            <div className="flex gap-10"><div className="text-right space-y-1"><div className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/50">Average Score</div><div className="text-3xl font-black tracking-tighter text-foreground tabular-nums">{totalPrecision}%</div></div><div className="text-right space-y-1"><div className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/50">Sessions</div><div className="text-3xl font-black tracking-tighter text-foreground tabular-nums">{pastPractices.length}</div></div></div>
          </div>
          <div className="grid grid-cols-12 gap-8">
            <div className="col-span-12 lg:col-span-8 space-y-8">
               <div className="grid grid-cols-3 gap-4">
                  <div className="p-6 bg-muted/10 border border-border rounded-md space-y-4 hover:border-primary transition-all group"><TrendingUp size={12} className="text-muted-foreground group-hover:text-primary"/><div className="space-y-1"><div className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/50">Score Stability</div><div className="text-xl font-black tracking-tighter text-foreground tabular-nums">{stability}</div></div></div>
                  <div className="p-6 bg-muted/10 border border-border rounded-md space-y-4 hover:border-primary transition-all group"><ScanSearch size={12} className="text-muted-foreground group-hover:text-primary"/><div className="space-y-1"><div className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/50">Suggested Next</div><div className="text-[10px] font-black tracking-tight text-foreground truncate uppercase">{suggested}</div></div></div>
                  <div className="p-6 bg-muted/10 border border-border rounded-md space-y-4 hover:border-primary transition-all group"><Dna size={12} className="text-muted-foreground group-hover:text-primary"/><div className="space-y-1"><div className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/50">Level</div><div className="text-xl font-black tracking-tighter text-foreground tabular-nums">{gapSeverity}</div></div></div>
               </div>
        ...
        <div className="max-w-4xl mx-auto px-8 py-20 w-full space-y-16">
          <div className="flex items-end justify-between border-b border-border pb-10">
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-[10px] font-black text-muted-foreground uppercase tracking-widest"><button onClick={() => setView('dashboard')} className="hover:text-foreground">Cancel</button><ChevronRight size={10} className="opacity-20"/><span className="text-foreground">Setup Session</span></div>
              <h1 className="text-4xl font-black tracking-tighter uppercase leading-none">Practice<br/><span className="text-muted-foreground/30">Setup</span></h1>
            </div>
            <div className="flex flex-col items-end gap-2"><span className="text-[10px] font-black text-muted-foreground/50 uppercase tracking-widest">Questions Ready</span><div className="text-2xl font-black text-primary tracking-tighter">{totalQuestions}</div></div>
          </div>
          <div className="flex flex-col gap-16">
            <div className="space-y-12">
               <div className="space-y-6">
                 <div className="flex items-center gap-3"><span className="text-[10px] font-black text-muted-foreground/30">01</span><h3 className="text-[10px] font-black uppercase tracking-widest text-foreground">Topic & Level</h3></div>
                 <div className="space-y-4">
                    <Label className="text-[10px] font-black uppercase text-muted-foreground/50">Neural Cluster</Label>
                    <Select value={selectedHub} onValueChange={setSelectedHub}>
                        <SelectTrigger className="w-full h-14 bg-muted/10 border-border rounded-md px-6 text-left uppercase focus:ring-primary"><div className="flex flex-col gap-0.5"><span className="text-[8px] text-muted-foreground/40">{hubs.find(h => h.id === selectedHub)?.course || "TOPIC"}</span><div className="text-[11px] font-black text-foreground"><SelectValue placeholder="Select topic..." /></div></div></SelectTrigger>
                 ...
                 <div className="space-y-4">
                    ...
                    <RadioGroup value={advancedConfig.difficulty} onValueChange={(val) => setAdvancedConfig(prev => ({ ...prev, difficulty: val as any }))} className="grid grid-cols-2 gap-3">
                        ...
                        <div key={level}><RadioGroupItem value={level} id={level} className="peer sr-only" /><Label htmlFor={level} className="flex flex-col items-center justify-center gap-1 h-16 border border-border rounded-md bg-background peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-muted/30 cursor-pointer hover:bg-muted/10 transition-all text-center"><span className="text-[11px] font-black uppercase tracking-widest">{level}</span><span className="text-[7px] text-muted-foreground/50 font-black uppercase tracking-widest">Level</span></Label></div>
                        ))}
                    </RadioGroup>
                 </div>
               </div>
               <div className="space-y-6">
                 <div className="flex items-center gap-3"><span className="text-[10px] font-black text-muted-foreground/30">02</span><h3 className="text-[10px] font-black uppercase tracking-widest text-foreground">Time Control</h3></div>
          ...
          <div className="pt-10 border-t border-border flex flex-col gap-10">
             ...
             <Button onClick={handleStartSession} disabled={isLoading} className="h-20 w-full bg-primary text-primary-foreground text-[10px] font-black uppercase tracking-[0.4em] rounded-md shadow-xl shadow-primary/10 hover:brightness-110 active:scale-[0.98] transition-all">
                {isLoading ? "CALIBRATING..." : "START PRACTICE"}
             </Button>
          </div>
        </div>
      </div>
        ...
        <div className="h-full flex-1 flex flex-col items-center justify-center bg-background text-foreground animate-in fade-in duration-500">
            <div className="flex flex-col items-center gap-10">
                ...
                <div className="text-center space-y-2">
                    <div className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.5em] animate-pulse">Initializing Interface</div>
                    <h2 className="text-2xl font-black uppercase tracking-tighter">Preparing Practice Session...</h2>
                </div>
      ...
      <div className="h-full flex-1 flex flex-col w-full bg-background text-foreground font-sans overflow-hidden animate-in fade-in duration-700">
        <div className="px-10 py-5 border-b border-border flex items-center justify-between bg-background/80 backdrop-blur-3xl z-20 shrink-0">
          <div className="flex items-center gap-10">
            <div className="space-y-1"><span className="text-[9px] font-black text-muted-foreground/50 uppercase tracking-widest">Practice Session</span><div className="text-[11px] font-black uppercase tracking-widest text-foreground">{hubs.find(h => h.id === selectedHub)?.title || 'Topic'}</div></div>
          ...
          <div className="flex items-center gap-6 text-[10px] font-black uppercase tracking-widest">{globalTimeLeft !== null && <span className="tabular-nums px-3 py-1.5 border border-border rounded-md bg-muted/20 text-foreground">{Math.floor(globalTimeLeft / 60)}:{String(globalTimeLeft % 60).padStart(2, '0')}</span>}<span className="text-muted-foreground/50">Question <span className="text-foreground">{currentQuestionIdx + 1}</span> / {questions.length}</span></div>
        </div>
        <div className="flex-1 overflow-y-auto custom-scrollbar p-12 lg:p-20 relative">
          <div className="max-w-2xl mx-auto space-y-16 pb-64">
             ...
             <div className="space-y-4">
                ...
                {['short_answer', 'scenario', 'code'].includes(currentQuestion.type) && (<div className="space-y-10"><div className="p-8 bg-muted/5 border border-border rounded-md focus-within:border-primary transition-all"><textarea rows={6} disabled={isRevealed} className="w-full bg-transparent border-none focus:ring-0 text-[13px] font-black uppercase tracking-widest placeholder:text-muted-foreground/10 resize-none leading-relaxed text-foreground/90 uppercase" placeholder="Type your answer here..." value={userAnswers[currentQuestion.id] || ""} onChange={(e) => handleSelectAnswer(e.target.value)} /></div>{isRevealed && (<div className="p-8 border-l-4 border-primary bg-muted/10 space-y-4 animate-in slide-in-from-left-2 rounded-r-md"><div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-primary"><Filter size={10}/> Correct Answer</div><p className="text-[13px] font-black uppercase leading-relaxed tracking-widest text-foreground">{(currentQuestion as any).answer}</p></div>)}</div>)}
             </div>
          </div>
        </div>
        <div className="bg-background/80 backdrop-blur-3xl border-t border-border p-8 shrink-0 z-30"><div className="max-w-2xl mx-auto flex items-center justify-between"><Button variant="ghost" onClick={() => { if (confirm("End practice early?")) resetSession(); }} className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 hover:text-foreground">End Practice</Button><div className="flex items-center gap-4">{!isRevealed ? (<Button onClick={handleSubmitAnswer} disabled={!userAnswers[currentQuestion.id] && currentQuestion.type !== 'code'} className="h-12 px-10 bg-primary text-primary-foreground text-[10px] font-black uppercase tracking-widest rounded-md shadow-lg shadow-primary/10 hover:brightness-110 active:scale-[0.98] transition-all">Check Answer <Zap size={14}/></Button>) : (<div className="flex items-center gap-4">{!gradedAnswers[currentQuestion.id] && ['short_answer', 'scenario'].includes(currentQuestion.type) && (<div className="flex gap-2"><Button onClick={() => { setGradedAnswers(p => ({...p, [currentQuestion.id]: false})); nextQuestion(); }} variant="outline" className="h-12 border-border text-muted-foreground/50 hover:text-destructive text-[9px] font-black uppercase rounded-md tracking-widest">Incorrect</Button><Button onClick={() => { setGradedAnswers(p => ({...p, [currentQuestion.id]: true})); nextQuestion(); }} className="h-12 bg-foreground text-background text-[9px] font-black uppercase rounded-md tracking-widest">Correct</Button></div>)} {(gradedAnswers[currentQuestion.id] !== undefined || !['short_answer', 'scenario'].includes(currentQuestion.type)) && (<Button onClick={nextQuestion} className="h-12 px-12 bg-primary text-primary-foreground text-[10px] font-black uppercase tracking-widest rounded-md hover:brightness-110 transition-all">Next Question <ArrowRight size={14}/></Button>)}</div>)}</div></div></div>
      </div>
    );
  }
    ...
    return (
      <div className="h-full flex-1 flex flex-col w-full bg-background text-foreground font-sans overflow-hidden animate-in fade-in duration-700 flex flex-col items-center justify-center p-20">
         <div className="max-w-2xl mx-auto w-full text-center space-y-12">
            <div className="space-y-2"><h1 className="text-8xl font-black tracking-tighter leading-none text-foreground">{calculateScore().score}<span className="text-2xl text-muted-foreground/30 tracking-normal">%</span></h1><p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/50">Final Score</p></div>
            <div className="grid grid-cols-2 divide-x divide-border border-y border-border py-10"><div className="space-y-1"><div className="text-2xl font-black tracking-tighter text-foreground">{calculateScore().correct}/{calculateScore().total}</div><div className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40">Correct Answers</div></div><div className="space-y-1"><div className="text-2xl font-black tracking-tighter text-foreground">{calculateScore().score}%</div><div className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40">Score</div></div></div>
            <div className="flex gap-4"><Button onClick={() => setView('configuring')} className="flex-1 h-14 bg-primary text-primary-foreground text-[10px] font-black uppercase tracking-widest rounded-md">Practice Again</Button><Button variant="outline" onClick={() => setView('dashboard')} className="flex-1 h-14 border-border text-muted-foreground/50 text-[10px] font-black uppercase tracking-widest rounded-md border-dashed hover:text-foreground">Back to Dashboard</Button></div>
         </div>
      </div>
    );
  }
