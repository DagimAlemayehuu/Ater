...
function BatchTreeView({batches, processedNotes}: {batches: any[], processedNotes: any[]}) {
 ...
 <div key={batch.id} className="p-4 rounded-xl border border-border/10 bg-muted/5">
 <div className="flex items-center gap-3 mb-4 border-b border-border/10 pb-3">
 <span className="text-[10px] font-black uppercase tracking-widest text-foreground">Batch {batch.id}</span>
 </div>
 ...
 <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40 w-4">
 {isProcessed ? '[X]' : '[ ]'}
 </span>
 <span className={cn(
 "text-[11px] font-black uppercase tracking-widest truncate", 
 isProcessed ? "text-muted-foreground/30 line-through" : "text-foreground"
 )}>
 {note}
 </span>
 </div>
...
function OkaDashboard({onBack}: {onBack: () => void}) {
 ...
 {/* Curriculum Editor */}
 {isCurriculumReady && !activePlan && !processing && (
 <div className="mb-8 p-8 rounded-2xl bg-muted/5 border border-border/10 space-y-6 -95">
 ...
 {batchFeed.map(b => (
 <div key={b.batch} className="p-6 rounded-2xl border border-border/10 bg-muted/5 -95">
