...
# ══════════════════════════════════════════════════════════════════════════════
# 7. DocumentSynthesizer
# ══════════════════════════════════════════════════════════════════════════════

class TestDocumentSynthesizer:
    def test_find_overlap_clusters_empty_db(self, vault):
        from src.domains.ater.synthesizer import DocumentSynthesizer

        synth = DocumentSynthesizer(vault)
        clusters = synth.find_overlap_clusters()
        assert clusters == []

    def test_find_overlap_clusters_detects_similar_notes(self, vault):
        from src.domains.ater.synthesizer import DocumentSynthesizer

        db = AcademicDB(vault)
        # Create two very similar unit vectors
        v1 = np.array([1.0, 0.0, 0.0])
        v2 = np.array([0.99, 0.14, 0.0])
        v2 = v2 / np.linalg.norm(v2)

        db.save_embedding("Notes/A.md", "ha", v1.tolist())
        db.save_embedding("Notes/B.md", "hb", v2.tolist())

        synth = DocumentSynthesizer(vault)
        clusters = synth.find_overlap_clusters(threshold=0.90)
        assert len(clusters) > 0
        cluster_notes = clusters[0]["notes"]
        assert "Notes/A.md" in cluster_notes
        assert "Notes/B.md" in cluster_notes

    def test_find_overlap_clusters_ignores_dissimilar_notes(self, vault):
        from src.domains.ater.synthesizer import DocumentSynthesizer
        ...
        v1 = np.array([1.0, 0.0])
        v2 = np.array([0.0, 1.0])
        db.save_embedding("Notes/Ortho_A.md", "h1", v1.tolist())
        db.save_embedding("Notes/Ortho_B.md", "h2", v2.tolist())

        synth = DocumentSynthesizer(vault)
        clusters = synth.find_overlap_clusters(threshold=0.80)
        assert clusters == []

    @pytest.mark.asyncio
    async def test_generate_synthesis_with_mocked_llm(self, vault, secrets):
        from src.domains.ater.synthesizer import DocumentSynthesizer

        note_a = vault / "Notes" / "Replication.md"
        note_a.write_text("# Replication\nData is copied across nodes.", encoding="utf-8")
        note_b = vault / "Notes" / "Sharding.md"
        note_b.write_text("# Sharding\nData is partitioned across nodes.", encoding="utf-8")

        synth = DocumentSynthesizer(vault)

        mock_resp = MagicMock()
        mock_resp.content = "# Synthesis: Replication vs Sharding\n| Metric | Replication | Sharding |\n|---|---|---|\n| Strategy | Copy | Partition |"

        with patch("src.domains.ater.synthesizer.ModelFactory.get_model") as mock_factory:
            mock_llm = MagicMock()
            mock_llm.ainvoke = AsyncMock(return_value=mock_resp)
            mock_factory.return_value = mock_llm

            result = await synth.generate_synthesis(
                ["Notes/Replication.md", "Notes/Sharding.md"], secrets
            )

        assert "Synthesis" in result or "Replication" in result

    @pytest.mark.asyncio
    async def test_generate_synthesis_no_content(self, vault, secrets):
        from src.domains.ater.synthesizer import DocumentSynthesizer

        synth = DocumentSynthesizer(vault)
        result = await synth.generate_synthesis(["Notes/DoesNotExist.md"], secrets)
        assert result == "No content to synthesize."


# ══════════════════════════════════════════════════════════════════════════════
# 8. EssayEvaluator – Mocked LLM + VaultIndexer
# ══════════════════════════════════════════════════════════════════════════════

class TestEssayEvaluator:
    @pytest.mark.asyncio
    async def test_evaluate_essay_returns_evaluation_and_references(self, vault, secrets):
        from src.domains.ater.essay_evaluator import EssayEvaluator

        essay_text = "CAP theorem states that distributed systems cannot guarantee all three properties simultaneously."

        mock_eval_resp = MagicMock()
        mock_eval_resp.content = "**Grade: A**\n- Fact Check: Correct.\n- Depth: 90%\n- Logic: Sound."

        with patch("src.domains.ater.essay_evaluator.VaultIndexer") as MockIndexer:
            mock_indexer_inst = MagicMock()
            mock_indexer_inst.semantic_search.return_value = [
                {"path": "Notes/CAP.md", "title": "CAP Theorem", "similarity": 0.95}
            ]
            (vault / "Notes" / "CAP.md").write_text("# CAP Theorem\nConsistency, Availability, Partition.", encoding="utf-8")
            MockIndexer.return_value = mock_indexer_inst

            evaluator = EssayEvaluator(vault)

            with patch("src.domains.ater.essay_evaluator.ModelFactory.get_model") as mock_factory:
                mock_llm = MagicMock()
                mock_llm.ainvoke = AsyncMock(return_value=mock_eval_resp)
                mock_factory.return_value = mock_llm

                result = await evaluator.evaluate_essay(essay_text, "Distributed Systems", secrets)

        assert "evaluation" in result
        assert "references" in result
        assert result["evaluation"] == mock_eval_resp.content
        assert "Notes/CAP.md" in result["references"]

    @pytest.mark.asyncio
    async def test_evaluate_essay_no_vault_matches(self, vault, secrets):
        from src.domains.ater.essay_evaluator import EssayEvaluator

        mock_eval_resp = MagicMock()
        mock_eval_resp.content = "No relevant notes found. Cannot perform fact-check."
        ...
        with patch("src.domains.ater.essay_evaluator.VaultIndexer") as MockIndexer:
            ...
            with patch("src.domains.ater.essay_evaluator.ModelFactory.get_model") as mock_factory:
                ...
                result = await evaluator.evaluate_essay("Some text without vault matches", "General", secrets)

        assert result["references"] == []


# ══════════════════════════════════════════════════════════════════════════════
# 9. StudyPackManager – Export and Import roundtrip
# ══════════════════════════════════════════════════════════════════════════════

class TestStudyPackManager:
    def _setup_hub_and_notes(self, vault):
        """Creates a mock hub + notes structure for study pack tests."""
        hub_file = vault / "database" / "study planner" / "CS_Hub.md"
        hub_file.write_text(
            "---\ntitle: CS_Hub\ntype: Hub\n---\n\n## Connections\n- [[Replication]]\n- [[Sharding]]",
            encoding="utf-8",
        )
        note1 = vault / "database" / "study planner" / "Replication.md"
        note1.write_text("---\ntitle: Replication\ncourse: Distributed Systems\n---\nContent.", encoding="utf-8")
        note2 = vault / "database" / "study planner" / "Sharding.md"
        note2.write_text("---\ntitle: Sharding\ncourse: Distributed Systems\n---\nContent.", encoding="utf-8")
        return hub_file, [note1, note2]

    def test_export_pack_creates_valid_zip(self, vault, analytics_db, secrets):
        from src.domains.ater.study_packs import StudyPackManager

        self._setup_hub_and_notes(vault)
        pack_mgr = StudyPackManager(vault, analytics_db, secrets)
        output = vault / ".ater" / "exports" / "cs_hub.zip"

        pack_mgr.export_pack("CS_Hub", output)
    ...
    def test_export_pack_includes_note_files(self, vault, analytics_db, secrets):
        from src.domains.ater.study_packs import StudyPackManager

        self._setup_hub_and_notes(vault)
        pack_mgr = StudyPackManager(vault, analytics_db, secrets)
        output = vault / ".ater" / "exports" / "cs_notes.zip"

        pack_mgr.export_pack("CS_Hub", output)

        with zipfile.ZipFile(output, "r") as zf:
            names = zf.namelist()
            assert "Replication.md" in names
            assert "Sharding.md" in names

    def test_export_pack_includes_srs_metadata(self, vault, analytics_db, secrets):
        from src.domains.ater.study_packs import StudyPackManager

        self._setup_hub_and_notes(vault)
        srs = SRSEngine(analytics_db)
        srs.review("database/study planner/Replication.md", rating=4)

        pack_mgr = StudyPackManager(vault, analytics_db, secrets)
        output = vault / ".ater" / "exports" / "srs_meta.zip"
        pack_mgr.export_pack("CS_Hub", output)

        with zipfile.ZipFile(output, "r") as zf:
            with zf.open("pack_metadata.json") as f:
                meta = json.load(f)
        notes_meta = {n["filename"]: n for n in meta["notes"]}
        assert "Replication.md" in notes_meta
        assert notes_meta["Replication.md"]["srs_data"]["reps"] >= 1

    def test_import_pack_roundtrip(self, vault, analytics_db, secrets, tmp_path):
        from src.domains.ater.study_packs import StudyPackManager

        # Export from original vault
        self._setup_hub_and_notes(vault)
        pack_mgr = StudyPackManager(vault, analytics_db, secrets)
        output = vault / ".ater" / "exports" / "roundtrip.zip"
        pack_mgr.export_pack("CS_Hub", output)

        # Import into a fresh vault
        fresh_vault = tmp_path / "fresh_vault"
        fresh_vault.mkdir()
        (fresh_vault / "Inbox").mkdir()
        fresh_db = fresh_vault / "Inbox" / "ater_queue.db"
        fresh_secrets = AppSecrets(
            ai_provider="google",
            ai_key="mock-key",
            ai_model="gemini-2.0-flash",
            vault_path=str(fresh_vault),
            inbox_path=str(fresh_vault / "Inbox"),
            academic_path="Notes",
        )

        # VaultIndexer is imported locally inside import_pack — patch at source module
        with patch("src.domains.ater.vault_indexer.VaultIndexer") as MockVI:
            mock_vi_inst = MagicMock()
            mock_vi_inst.index_note.return_value = True
            MockVI.return_value = mock_vi_inst
            import_mgr = StudyPackManager(fresh_vault, fresh_db, fresh_secrets)
            result = import_mgr.import_pack(output)

        assert result["success"] is True
        assert result["hub_id"] == "CS_Hub"
        assert result["notes_imported_count"] >= 1

    def test_import_pack_invalid_zip_raises(self, vault, analytics_db, secrets, tmp_path):
        from src.domains.ater.study_packs import StudyPackManager
        ...
        with pytest.raises(Exception):  # zipfile.BadZipFile
            pack_mgr.import_pack(bad_zip)

    def test_export_pack_raises_for_unknown_hub(self, vault, analytics_db, secrets):
        from src.domains.ater.study_packs import StudyPackManager

        pack_mgr = StudyPackManager(vault, analytics_db, secrets)
        with pytest.raises(ValueError, match="not found"):
            pack_mgr.export_pack("NonExistentHub", vault / "out.zip")


# ══════════════════════════════════════════════════════════════════════════════
# 10. KnowledgeGapDetector – SRS-based gaps
# ══════════════════════════════════════════════════════════════════════════════
