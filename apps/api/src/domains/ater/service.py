...
class AterService:
    ...
    """
    _sessions = SessionStore._sessions
    _status = SessionStore._status
    _rate_limited = SessionStore._rate_limited
    _session_file = SessionStore._session_file
    _status_callback = SessionStore._status_callback
    ...
    def set_status(cls, session_id: str, message: str):
        ...
        SessionStore.set_status(session_id, message)

    @classmethod
    def register_status_callback(cls, callback):
        SessionStore.register_status_callback(callback)

    def _ensure_session_dir(self):
        SessionStore.ensure_session_dir()

    def _persist_session(self, session_id: str, data: Dict[str, Any]):
        """Saves session state to disk for recovery."""
        SessionStore.persist_session(session_id, data)

    async def _get_or_restore_session(self, session_id: str) -> Optional[Dict[str, Any]]:
        return await SessionStore.get_or_restore_session(session_id, self.resolve_si_path, self._get_si)
    ...
    def list_practices(self) -> List[Dict[str, Any]]:
        ...
        return _list_practices_ext(
            vm=self.vm,
            planner_path=self._get_planner_path(),
            academic_root=self.vm.academic_root,
            vault_path=self.vm.vault_path,
    ...
    def update_practice_score(self, practice_path: str, score: int) -> bool:
        ...
        return _update_practice_score_ext(practice_path, score)
    ...
    async def generate_practice(
        ...
        """
        return await _generate_practice_ext(
            ...
            swap_api_key_fn=self.swap_api_key,
            status_dict=SessionStore._status,
            hub_id=hub_id,
