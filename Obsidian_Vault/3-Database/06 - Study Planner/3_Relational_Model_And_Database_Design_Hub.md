---
title: 3_Relational_Model_And_Database_Design_Hub
type: Hub
course: [[Database Systems]]
semester: [[Autumn 2025]]
unit: 3
source: [[Chapter_3.Pdf]]
source_pages: []
status: Not Started
confidence: null
study_date: null
generated: true

---

# Learning Objectives
After mastering this unit, you can:
1. Construct an ER diagram from a requirements specification.
2. Map an ER schema to a complete set of relational tables.
3. Identify and resolve weak entities and their identifying relationships.

# Core Topologies (Connections)
- [[Database_Development_Methodology]]
  - [[Phases_Of_Database_Design]]
  - [[Entity–Relationship_Modelling]]
    - [[Entity_Types]]
      - [[Strong_Entity_Type]]
      - [[Weak_Entity_Type]]
    - [[Relationship_Types]]
      - [[Degree_Of_Relationship]]
      - [[Recursive_Relationship]]
      - [[Role_Names_In_Relationships]]
    - [[Attributes]]
      - [[Simple_Attribute]]
      - [[Composite_Attribute]]
      - [[Single_Valued_Attribute]]
      - [[Multi_Valued_Attribute]]
      - [[Derived_Attribute]]
    - [[Keys]]
      - [[Candidate_Key]]
      - [[Primary_Key]]
      - [[Composite_Key]]
  - [[Structural_Constraints]]
    - [[Cardinality]]
    - [[Participation]]
    - [[Multiplicity]]

# Assessment Layer
[[3_Relational_Model_And_Database_Design_Possible_Questions]]