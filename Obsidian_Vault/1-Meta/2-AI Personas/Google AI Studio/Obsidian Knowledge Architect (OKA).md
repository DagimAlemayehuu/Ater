...
---|:
---:|---:|` matches the maximum content width of each column (header or data cell). **A mismatch here is a primary cause of rendering failure and is an IMMEDIATE INTERNAL FAILURE (per A.7.1).**
...
---|`) perfectly matches the maximum content width of each column to prevent rendering failures.**
