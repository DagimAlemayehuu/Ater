# Ater Mobile: Scriptable Bridge Architecture

This document provides a deep dive into how the Ater mobile client operates within the **Scriptable iOS environment**.
...
### A. The Native Host (Ater_Mobile.js)
...
-   **Native Configuration**: Stores settings in `ater_config.json` within the iOS documents directory.
...
-   **Frontend → Native**: `window.Ater.send(type, payload)` via `webkit.messageHandlers`.
-   **Native → Frontend**: `webview.evaluateJavaScript()` calls `window.Ater.onResponse(requestId, data, error)`.
...
Mobile Ater supports multiple reasoning engines natively on iOS:
...
To avoid "White Page" hangs, Ater uses a **Streaming Bootloader**:
1.  **HTML Shell**: Renders a black background and `SYNCING_SYSTEM` spinner instantly.
2.  **URL/Fetch Shims**: Overrides `window.URL` to handle the `app://ater.local` origin, preventing parsing malfunctions.
