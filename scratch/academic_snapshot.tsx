                                        ...
                                        <AssignmentsDashboard 
                                            ...
                                            onUpdate={(id:string, key:string, val:any) => {
                                                ...
                                                if (id && key && val !== undefined) {
                                                    ...
                                                    handleUpdateRow("03 - Assignments", id, key); // fallback to old signature
                                                }
                                            }}
...
function YearsDashboard({ data, databases, onSelect, selectedId, onUpdate, onCreate, onDelete }: any) {
    ...
    const chartData = [
        { name: 'Completed', value: completedYears, color: 'hsl(var(--foreground))' },
        { name: 'Active', value: activeYears, color: 'hsl(var(--primary))' },
        { name: 'Pending', value: max(0, 4 - total), color: 'hsl(var(--muted))' },
    ];

    return (
        <div className="space-y-12">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="p-8 border border-border/10 rounded-2xl bg-muted/5 flex flex-col items-center justify-center space-y-4">
                    <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Degree Completion</span>
                    <div className="h-32 w-32 relative">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center text-xl font-black text-foreground">
                            {Math.round((completedYears / (completedYears + activeYears || 1)) * 100)}%
                        </div>
                    </div>
                </div>
                <div className="col-span-2 grid grid-cols-2 gap-6">
                    <div className="p-8 border border-border/10 rounded-2xl bg-muted/5 flex flex-col justify-between">
                        <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Total Years</span>
            ...
            <UniversalDatabaseTab dbId="09 - Years" data={data} databases={databases} selectedId={selectedId} onSelect={onSelect} onUpdate={onUpdate} onCreate={onCreate} onDelete={onDelete} />
        </div>
    )
}
