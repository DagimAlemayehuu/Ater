...
def generate_mocs(all_notes_metadata: list, output_root_path: Path) -> Tuple[bool, List[Path], List[str]]:
    ...
    cs_moc_path = output_root_path / "Database" / "00 - Meta" / "Mocs" / f"{vault_utils.sanitize_filename(cs_moc_title_canonical)}.md"
    ...
    for year, semesters_data in notes_by_year.items():
        ...
        moc_path = output_root_path / "Database" / "00 - Meta" / "Mocs" / "Years" / f"{vault_utils.sanitize_filename(moc_title_canonical)}.md"
    ...
    for sem_key, notes in notes_by_semester.items():
        ...
        moc_path = output_root_path / "Database" / "00 - Meta" / "Mocs" / "Semesters" / f"{vault_utils.sanitize_filename(moc_title_canonical)}.md"
        moc_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    for course_key, notes in notes_by_course.items():
        ...
        moc_path = output_root_path / "Database" / "00 - Meta" / "Mocs" / "Courses" / f"{vault_utils.sanitize_filename(moc_title_canonical)}.md"
        moc_path.parent.mkdir(parents=True, exist_ok=True)
