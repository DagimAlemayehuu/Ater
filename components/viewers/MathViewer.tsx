...
import { Copy, Check, FunctionSquare, Maximize2 } from 'lucide-react';
...
export const MathViewer: React.FC<MathViewerProps> = ({
  ...
  return (
    <div
      ...
      className={`my-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 overflow-hidden shadow-xs ${className}`}
    >
      {/* Header */}
          ...
          {!disableExpand && (
            <>
              ...
              <button
                ...
                aria-label="Expand math formula"
                title="Expand math formula"
                className="p-1 rounded text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-200/60 dark:hover:bg-zinc-800 transition-colors flex items-center gap-1"
              >
      ...
      <div
        ...
        title={!disableExpand ? 'Click to expand formula' : undefined}
        className={`p-5 overflow-x-auto text-center flex items-center justify-center text-zinc-900 dark:text-zinc-100 text-sm sm:text-base ${
          !disableExpand ? 'cursor-pointer hover:bg-zinc-50/50 dark:hover:bg-zinc-900/60 transition-colors' : ''
        }`}
      >
      ...
      {/* Expanded Modal View without unnecessary lines or source dumping */}
      {!disableExpand && isExpanded && (
        <ArtifactModal
          isOpen={isExpanded}
          onClose={() => setIsExpanded(false)}
          badgeText="Formula"
          title={caption || 'Mathematical Formula & Proof'}
        >
          <div className="flex flex-col items-center justify-center min-h-[55vh] p-6 text-base sm:text-2xl text-zinc-900 dark:text-zinc-100 overflow-x-auto">
            <div dangerouslySetInnerHTML={{ __html: renderedHtml }} />
          </div>
        </ArtifactModal>
      )}
